            ...
            {col cols="auto"}
                {breadcrumb id="breadcrumb" itemprop="breadcrumb" itemscope=true itemtype="https://schema.org/BreadcrumbList"}
                    {block name='layout-breadcrumb-sm-back'}
                        ...
                        {$lastVisitedProductListURL = $smarty.session.lastVisitedProductListURL|default:''}
                        {if $nSeitenTyp === $smarty.const.PAGE_ARTIKEL
                        && $lastVisitedProductListURL !== ''
                        && $Brotnavi[array_key_last($Brotnavi)]->getID() > 0}
                            {breadcrumbitem class="breadcrumb-backtolist"
                            href=$lastVisitedProductListURL|cat:'#buy_form_'|cat:$Brotnavi[array_key_last($Brotnavi)]->getID()}
