{block name='layout-header-shop-nav-account'}
    {navitemdropdown tag="li"
        ...
        no-caret=true
        right=true
        text="{if JTL\Session\Frontend::getCustomer()->getID() === 0}<span class=\"fas fa-user\" title=\"{lang key='myAccount'}\"></span>{else}<span class=\"fas fa-user-check\" title=\"{lang key='myAccount'}\"></span>{/if}"
