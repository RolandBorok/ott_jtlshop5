        ...
        {col cols=12 lg=3 class="cfg-item-qty"}
            ...
            {else}
                {if !empty($nKonfigitemAnzahl_arr[$oItem->getKonfigitem()])}
                    {$itemQuantityVal = $nKonfigitemAnzahl_arr[$oItem->getKonfigitem()]}
                {else}
                    {if $oItem->getArtikel()->fAbnahmeintervall > 0}
                        {if $oItem->getArtikel()->fMindestbestellmenge > $oItem->getArtikel()->fAbnahmeintervall}
                            {$itemQuantityVal = $oItem->getArtikel()->fMindestbestellmenge}
                        {else}
                            {$itemQuantityVal = $oItem->getArtikel()->fAbnahmeintervall}
                        {/if}
                    {else}
                        {if ($oItem->getInitial()>0)}
                            {$itemQuantityVal = $oItem->getInitial()}
                        {else}
                            {$itemQuantityVal = $oItem->getMin()}
                        {/if}
                    {/if}
                {/if}
