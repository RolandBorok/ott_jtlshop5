            ...
            {if $Suchergebnisse->getPages()->getMaxPage() > 1 && !($isMobile && $navid === 'header')}
                {block name='snippets-productlist-page-nav-page-nav'}
                    {col cols=12 md="auto" class="productlist-pagination"}
                        <nav class="navbar-pagination" aria-label="{lang section='aria' key='productlistNavigation'}">
                            <ul class="pagination">
                                {block name='snippets-productlist-page-nav-first-page'}
                                    <li class="page-item{if $Suchergebnisse->getPages()->getCurrentPage() == 1} disabled{/if}">
                                        {link class="page-link js-pagination-ajax" href=$filterPagination->getPrev()->getURL()|default:"#" aria=['label' => {lang key='previous' section='productOverview'}, 'disabled' => "{if $Suchergebnisse->getPages()->getCurrentPage() == 1}true{else}false{/if}"]}<i class="fas fa-long-arrow-alt-left"></i>{/link}
                                    </li>
                                {/block}
                                <li class="page-item dropdown">
                                    {block name='snippets-productlist-page-nav-button'}
                                        <button type="button" class="btn btn-outline-secondary dropdown-toggle" data-toggle="dropdown" tabindex="0" aria-haspopup="true" aria-expanded="false" aria-controls="dropdown-menu-pagination-{$navid}">
                                            <span class="pagination-site">{lang key='page'}</span> {$Suchergebnisse->getPages()->getCurrentPage()}
                                        </button>
                                    {/block}
                                    <div id="dropdown-menu-pagination-{$navid}" class="dropdown-menu shadow-none">
                                ...
                                {block name='snippets-productlist-page-nav-last-page'}
                                    <li class="page-item{if $Suchergebnisse->getPages()->getCurrentPage() == $Suchergebnisse->getPages()->getMaxPage()} disabled{/if}">
                                        {link class="page-link js-pagination-ajax" href=$filterPagination->getNext()->getURL()|default:"#" aria=['label' => {lang key='next' section='productOverview'}, 'disabled' => "{if $Suchergebnisse->getPages()->getCurrentPage() == $Suchergebnisse->getPages()->getMaxPage()}true{else}false{/if}"]}<i class="fas fa-long-arrow-alt-right"></i>{/link}
                                    </li>
                                {/block}
                            </ul>
                        </nav>
