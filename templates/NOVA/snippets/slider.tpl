{block name='snippets-slider'}
    {if isset($oSlider) && count($oSlider->getSlides()) > 0}
        {container fluid=$isFluid class="{if $Einstellungen.template.theme.left_sidebar === 'Y' && $boxesLeftActive}container-plus-sidebar{/if}"}
            ...
            <div class="slider-wrapper theme-{$oSlider->getTheme()}{if $oSlider->getControlNav()} control-nav{/if}{if $oSlider->getDirectionNav()} direction-nav{/if}{if $oSlider->getThumbnail()} thumbnail-nav{/if}{if $oSlider->getUseKB()} ken-burns-active{/if}">
        ...
        {block name='snippets-slider-script'}
            {inline_script}<script>
                function addPauseButton(slider) {
                    let pauseButton = $('<button tabindex="0" class="btn btn-secondary btn-sm slider-pause-btn" title="{lang key='pause' section='aria'}" aria-label="{lang key='pause' section='aria'}"><i class="fas fa-pause"></i></button>');

                    const sliderWrapper = slider.parent();
                    const isBar = sliderWrapper.hasClass('theme-bar');
                    const isKen = sliderWrapper.hasClass('ken-burns-active');
                    const hasThumbs = sliderWrapper.hasClass('thumbnail-nav');

                    if (isBar && !isKen) {
                        let container = $('<div></div>')
                            .addClass('slider-pause-controls slider-pause-bar-theme')
                            .css('margin-top', '10px')
                            .append(pauseButton);
                        sliderWrapper.after(container);
                    } else {
                        let container = $('<div></div>')
                            .addClass('slider-pause-controls')
                            .toggleClass('slider-pause-overlay', !isKen && !hasThumbs)
                        ...
                        if (isKen) {
                            ...
                            let controlNav = sliderWrapper.find('.nivo-controlNav');
                            if (controlNav.length) {
                                controlNav.before(container);
                            } else {
                    ...
                    pauseButton.on('click', function () {
                        ...
                        let isPaused = fa.hasClass('fa-pause');
                        if (isPaused) {
                            sliderInstance.stop();
                            fa.removeClass('fa-pause').addClass('fa-play');
                            pauseButton.attr('title', '{lang key='play' section='aria'}')
                            .attr('aria-label', '{lang key='play' section='aria'}');
                        } else {
                            sliderInstance.start();
                            fa.addClass('fa-pause').removeClass('fa-play');
                            pauseButton.attr('title', '{lang key='pause' section='aria'}')
                            .attr('aria-label', '{lang key='pause' section='aria'}');
                            }
                        });
                    }
