{block name='productdetails-price'}
    {if $smarty.session.Kundengruppe->mayViewPrices()}
        <div class="price_wrapper">
                        {if $Einstellungen.artikeldetails.artikeldetails_uvp_anzeigen === 'Y' && ($tplscope == 'detail')
                            && $Artikel->fUVP > 0
                            && $Artikel->Preise->fVKBrutto < $Artikel->fUVP} <div class="uvp-gross"><span class="percent">{lang key='youSave' section='productDetails'} {$Artikel->SieSparenX->nProzent}%</span></div>{/if}
            ...
            {elseif $Artikel->Preise->fVKNetto == 0 && $Einstellungen.global.global_preis0 === 'N'}
                {block name='productdetails-price-on-application'}
                    <span style="background:#ba0109;color:#fff;padding:5px 12px;display:inline-block;" class="price_label price_on_application">{lang key='priceOnApplication'}</span>
                {/block}
            {else}
                ...
                <div class="price {if $priceLarge|default:false}h1{else}productbox-price{/if} {if isset($Artikel->Preise->Sonderpreis_aktiv) && $Artikel->Preise->Sonderpreis_aktiv} special-price{/if}">
                    {block name='productdetails-range'}
                        ...
                        {if $tplscope !== 'detail' && $Artikel->Preise->oPriceRange->isRange()}
                            ...
                            {/if}
                        {else}
                            {if $Artikel->Preise->oPriceRange->isRange() && ($Artikel->nVariationsAufpreisVorhanden == 1 || $Artikel->bHasKonfig) && $Artikel->kVaterArtikel == 0}{$Artikel->Preise->cVKLocalized[$NettoPreise]}{else}{$Artikel->Preise->cVKLocalized[$NettoPreise]}{/if}
                        {/if}
                            ...
                            && $Artikel->Preise->fVKBrutto < $Artikel->fUVP}
                                {block name='productdetails-price-uvp'}
                                    ...
                                    {if isset($Artikel->SieSparenX) && $Artikel->SieSparenX->anzeigen == 1 && $Artikel->SieSparenX->nProzent > 0 && !$NettoPreise && $Artikel->taxData['tax'] > 0}
                                        <div class="yousave">(Sie sparen <span class="value text-nowrap-util">{$Artikel->SieSparenX->cLocalizedSparbetrag}</span>)
                                        </div>
                                    {/if}
                                {/block}
                            {/if}
