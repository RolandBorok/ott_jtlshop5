                                ...
                                {tab title="{lang key="description" section="productDetails"}" active=$setActiveClass.description id="description"}
                                    {block name='productdetails-tabs-tab-content'}
                                        {block name='tab-description-media-types'}
                                            ...
                                            </div><a id="more">Weiterlesen ></a><a id="less">< Weniger</a>
...
{if (isset($Artikel->FunktionsAttribute['sicherheitshinweiseee']) && $Artikel->FunktionsAttribute['sicherheitshinweiseee']|@strlen > 0)}
{block name='productdetails-tabs-tab-sicherheitshinweise'}
{tab title="Sicherheitshinweise" active=$setActiveClass.sicherheitshinweise id="sicherheitshinweise"}
<a class="popup-dep" href="#" id="sh" title="{$Artikel->FunktionsAttribute.sicherheitshinweise}"><img style="max-width:100%;" alt="{$Artikel->FunktionsAttribute.sicherheitshinweise}" src="/mediafiles/Datenblaetter/sicherheitshinweise/{$Artikel->FunktionsAttribute.sicherheitshinweise}.jpg" /></a>
<div class="hidden" id="popupsh"><img alt="{$Artikel->FunktionsAttribute.sicherheitshinweise}" class="center-block" style="max-width:100%;" src="/mediafiles/Datenblaetter/sicherheitshinweise/{$Artikel->FunktionsAttribute.sicherheitshinweise}.jpg" /></div>	
{/tab}
{/block}
{/if}                   
...
{/if}{assign var='gpsr_responsibleperson' value=$gpsrData[$Artikel->kArtikel]['responsibleperson']}
{if count($gpsr_responsibleperson) > 0}
<div class="product-manufacturer mb-3">
    <strong>EU-{$langGPSRRespPersonLabel}:</strong>
